...
2. **温度控制**（一致性优化）：
   - **目标**: 基于实际仿真数据的温度一致性优化
   - **策略**: 针对22-36°C实际温度范围的多维度优化
   - **异常值检测与惩罚**（基于实际数据调整）:
  - 正常范围: 25-36°C（适应V27的24-25°C低温现象）
  - 低温异常: <25°C - 减轻惩罚力度
  - 高温异常: >36°C - 减轻惩罚力度
   - **核心一致性指标**（基于实际仿真数据优化）:
      - **标准差**: 权重35%，针对实际2.9°C标准差优化
        - 优秀: ≤2.0°C, 良好: ≤3.0°C（包含实际值）, 可接受: ≤4.0°C
      - **四分位距(IQR)**: 权重25%，基于实际数据调整
        - 优秀: ≤3.0°C, 良好: ≤4.5°C, 可接受: ≤6.0°C
      - **极差**: 权重20%，针对实际10-11°C范围优化
        - 优秀: ≤8.0°C, 良好: ≤11.0°C（包含实际值）, 可接受: ≤14.0°C
   - **分布特征**（基于实际数据优化）:
       - **温度聚类**: 权重10%，奖励28-35°C理想区间占比（扩大范围）
       - **分布均匀性**: 权重10%，Gini系数阈值放宽至0.15，减轻惩罚
       - **移除空间相关性**: 阀门编号不代表实际空间位置
   - **综合评估**: 多指标加权平均，异常值额外惩罚
...
3. **供水温度合理性**：
 ...
 4. **能耗控制**：
   - 阀门开度在50-100%范围内，越高越好
   - 使用平滑sigmoid函数计算奖励，开度越高奖励越高
   - 不再分区间限制动作变化幅度，让策略自由决定
   - 添加动作平滑性奖励，避免相邻动作变化过大
